...
> **⚠️ Redis 数据与 Reuse**：复用模式下 Redis 中的数据跨 JVM 持久存在。测试需在 `@BeforeEach`/`@AfterEach` 中通过统一前缀（如 `DT_TEST_`）管理 Key，避免数据污染。
...
3. 按需 `@Autowired` 注入 RedisTemplate/RedisService
4. `@BeforeEach` 准备数据，`@AfterEach` 清理数据（统一前缀清理）
